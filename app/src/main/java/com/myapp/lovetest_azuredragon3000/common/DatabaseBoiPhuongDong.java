...
public class DatabaseBoiPhuongDong extends SQLiteOpenHelper {
    ...
    public static final String DATABASE_NAME = "boiphuongdong.db";

    private DatabaseBoiPhuongDong(Context context) {
    ...
    public static DatabaseBoiPhuongDong getInstance(Application app){
        if (INSTANCE == null) {
            synchronized (DatabaseBoiPhuongDong.class) {
                if (INSTANCE == null) {
                    INSTANCE = new DatabaseBoiPhuongDong(app);
                }
            }
        }
        return INSTANCE;
